...
# 다양한 레시피 데이터 (메인 재료 + 재료 + 조리법 + 난이도 + 조리시간)
def get_recipes():
    return {
        "김치볶음밥": {
            "main_ingredient": "밥",
...
if st.button("레시피 추천받기"):
    if not user_ingredients:
        ...
        # 레시피 추천 로직: 메인 재료가 있어야 추천
        scored_recipes = []
        for recipe, data in recipes.items():
            if data['main_ingredient'] in user_ingredients:
        ...
        if scored_recipes:
            ...
            st.warning("❌ 선택한 재료에 메인 재료가 포함되지 않아 추천할 레시피가 없습니다.")
